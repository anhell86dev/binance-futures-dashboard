#!/usr/bin/env python3
"""Binance Futures Account API V1.1"""
...
def signed_request(endpoint: str, method: str = 'GET', params: Optional[Dict] = None) -> Dict:
    ...
    if method == 'GET': url += f"?{query_string}&signature={signature}"
...
def get_position_risk() -> List[Dict]:
    try:
        ...
        return [{'symbol': p['symbol'], 'side': 'LONG' if p['positionSide'] == 'LONG' else 'SHORT',
                'size': float(p['positionAmt']), 'entryPrice': float(p['entryPrice']),
                'markPrice': float(p['markPrice']), 'unrealizedPnL': float(p['unRealizedProfit']),
                'leverage': int(p['leverage']), 'marginType': 'Isolated' if p['marginType'] == 'isolated' else 'Cross',
                'liquidationPrice': float(p['liquidationPrice']), 'notional': float(p['notional']),
...
def get_account_info() -> Optional[Dict]:
    try:
        ...
        assets = [{'asset': a['asset'], 'walletBalance': float(a['walletBalance']),
                  'unrealizedProfit': float(a['unrealizedProfit']), 'marginBalance': float(a['marginBalance']),
                  'availableBalance': float(a['availableBalance'])
                 } for a in info['assets'] if float(a['walletBalance']) > 0 or float(a['unrealizedProfit']) != 0]
        return {'totalWalletBalance': float(info['totalWalletBalance']),
                'totalUnrealizedProfit': float(info['totalUnrealizedProfit']),
                'totalMarginBalance': float(info['totalMarginBalance']),
                'availableBalance': float(info['availableBalance']), 'assets': assets}
    except Exception as e:
        print(f"Error: {e}")
        return None
...
def get_open_orders() -> List[Dict]:
    try:
        ...
        return [{'symbol': o['symbol'], 'orderId': o['orderId'], 'side': o['side'], 'type': o['type'],
                'price': float(o['price']), 'quantity': float(o['origQty']),
                'executedQty': float(o['executedQty']), 'status': o['status'],
...
def get_all_account_data() -> Dict:
    ...
    data = {'success': True, 'timestamp': datetime.utcnow().isoformat(), 'account': account,
            'positions': positions, 'openOrders': orders,
            'summary': {'totalPositions': len(positions), 'totalOpenOrders': len(orders),
